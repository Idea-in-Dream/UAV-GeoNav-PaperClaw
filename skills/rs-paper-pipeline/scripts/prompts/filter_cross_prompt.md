...
5. 传统或通用 SLAM、视觉里程计（VO）、视觉惯性里程计（VIO）、视觉 SLAM 或 LiDAR-惯性里程计，只要主要输出为相机/机器人位姿、轨迹或地图，且方法可迁移到无人机；不再强制要求标题或摘要明确出现 UAV；
...
- 明确依赖非空中平台特有约束、且难以迁移到无人机的方法，例如依赖人形机器人关节编码器、道路车辆轮速或水下声呐的专用里程计；
- 仅卫星影像之间的配准；
