...
今日遥感AI研究聚焦灾害应急与自主导航两大方向。一方面，多模态大模型被用于灾害地名的消歧与跨视角地理定位，以提升灾害信息提取的准确性；另一方面，面向长距离越野导航的可通行性感知全局规划器被提出，结合卫星与无人机数据，增强复杂环境下的自主决策能力。两项工作均注重数据集构建与可复现性，推动遥感AI在动态场景中的实际应用。

## ✨ 今日亮点

- 多模态大模型结合跨视角定位，提升灾害地名消歧精度。
- 可通行性感知全局规划器，增强长距离越野导航鲁棒性。
- 两项研究均发布数据集或基准，促进领域可复现发展。
...
- 灾害应急领域开始融合大模型与地理定位技术，提升信息提取的语义理解能力。
- 自主导航研究更注重全局规划与可通行性建模，结合卫星与地面视角数据。
