...
- 无人机及可迁移到无人机的通用 SLAM、视觉里程计（VO）、视觉惯性里程计（VIO）与 LiDAR-惯性里程计；
- 无人机自身定位与目标地理定位；
- 热红外无人机与卫星/地图的跨模态定位；
- 直接相关的数据集、基准、尺度恢复和复现资源。

普通遥感检测/分割/变化检测、纯 UAV 检测/跟踪/规划、依赖人形关节编码器或道路轮速等非空中平台特有约束的专用里程计，以及仅卫星影像之间的配准会被排除。不确定条目会保留并标记 `Needs-Review`。
...
| `ARXIV_API_PROXY_PREFIX` | 否 | 可选的 arXiv 只读代理前缀；系统先直连官方 Atom API，失败时自动改用带完整摘要的 arXiv 高级搜索页，仅在显式配置时使用第三方代理 |
...
即仅在周二至周六 UTC 00:07（北京时间 08:07）运行，周日、周一不自动更新论文。每次处理前一个自然日，因此对应处理周一至周五的论文数据。避开 UTC 整点高峰可减少 GitHub Actions 排队延迟，但平台仍不保证秒级准时。LLM 请求连续失败时会自动降级为关键词筛选，将可能相关论文标记为 `Needs-Review`，不再直接中断日报。工作流会依次：
